            ...
            <div class="row">
                <div style="width: 100%; margin-top: 50px;">
                    <table class="table">
                        ...
                        <tbody>
                            @foreach ($todeliver as $todelivers )
                            <tr>
                                <td>
                                    <p>{{ $todelivers->OrderID }}</p>
                                </td>
                                <td>
                                    <p>{{ $todelivers->CustomerID }}</p>
                                </td>
                                <td>
                                    <p>{{ $todelivers->TotalAmount  }}</p>
                                </td>
                                <td>
                                    <p>{{ $todelivers->ShippingAddress }}</p>
                                </td>
                                <td>
                                    <p>{{ $todelivers->PaymentMethod  }}</p>
                                </td>
                                <td>
                                    <form action="{{ route('toReceiveViews') }}" method="post">
                                        @csrf
                                        <input type="hidden" name="orderID" value="{{ $todelivers->OrderID }}">
